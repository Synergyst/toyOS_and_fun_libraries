...
static volatile bool g_zmodem_active = false;
#include "ZModem.h"
...
static void handleCommand(char* line) {
  ...
  } else if (!strcmp(t0, "rz")) {
    char* optName = nullptr;
    (void)nextToken(p, optName);  // optional; if null, uses name from sender
    if (optName && strlen(optName) > ActiveFS::MAX_NAME) {
      Console.println("rz: name too long (<=32)");
      return;
    }
    bool ok = ZModem::receiveToFS(optName);
    Console.println(ok ? "rz: OK" : "rz: failed");
    return;
  } else {
...
static void pumpConsole() {
  ...
  if (g_zmodem_active) return;  // MUTE console during ZMODEM receive
  ...
  for (;;) {
    ...
    if (readByte(b, 50)) {
      ...
      if (w[0] == magicBytes[0] && w[1] == magicBytes[1] && w[2] == magicBytes[2] && w[3] == magicBytes[3]) {
        ...
        if (hdr.len) {
          if (!readExact(payloadBuf, hdr.len, 2000)) {
...
void setup() {
  ...
  performance_test();
  MCP.fastWriteA(0);
  if (!Console.tftAttach(&SPI1, 20, 21, -1, 240, 320, 1)) {
