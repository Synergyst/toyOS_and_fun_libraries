// main_mcu.ino - Multi-arch (RP2040 and ESP32-S3) compatible
...
#define UNIFIED_SPI_CLOCK_HZ 40000000UL  // 40 MHz SPI
#define W25Q_SPI_CLOCK_HZ 40000000UL     // 40 MHz NOR
#define MX35_SPI_CLOCK_HZ 40000000UL     // 40 MHz NAND
...
static void disableAllWatchdogsESP32() {
  disableLoopWDT();
  //disableCore0WDT();
  //disableCore1WDT();
  //disableWiFiWDT();
  (void)esp_task_wdt_deinit();  // Fully deinit Task WDT (ignore error if already deinitialized)
}
#endif
...
// ========== Cross-arch helpers for memory info and reboot ==========
static void printMemInfo() {
  ...
  Console.println();
  Console.printf("Total Heap:        %u bytes\n", (unsigned)heap_total);
  Console.printf("Free Heap:         %u bytes\n", (unsigned)heap_free);
  Console.printf("Used Heap:         %u bytes\n", (unsigned)heap_used);
  Console.printf("Total PSRAM Heap:  %u bytes\n", (unsigned)psram_total);
  Console.printf("Free PSRAM Heap:   %u bytes\n", (unsigned)psram_free);
  Console.printf("Used PSRAM Heap:   %u bytes\n", (unsigned)psram_used);
#else
  Console.println();
  Console.println("(meminfo not implemented for this core)");
#endif
}
...
static void doBootloader() {
#if defined(ARDUINO_ARCH_RP2040)
  rp2040.rebootToBootloader();
#elif defined(ARDUINO_ARCH_ESP32)
  // No dedicated "bootloader" mode; do regular restart
  ESP.restart();
#else
  NVIC_SystemReset();
#endif
}
static void configureSharedSPIForMCP() {
#if defined(ARDUINO_ARCH_RP2040)
  // RP2040 supports setSCK/MOSI/MISO prior to begin()
  SPI.setSCK(PIN_FLASH_SCK);
  SPI.setMOSI(PIN_FLASH_MOSI);
  SPI.setMISO(PIN_FLASH_MISO);
  SPI.begin();
#elif defined(ARDUINO_ARCH_ESP32)
  // ESP32: map pins by calling begin(sck, miso, mosi, ss)
  SPI.end();
  SPI.begin(PIN_FLASH_SCK, PIN_FLASH_MISO, PIN_FLASH_MOSI, -1);
#else
  SPI.begin();  // best-effort fallback
#endif
}
...
static void handleCommand(char* line) {
  ...
  if (!strcmp(t0, "help")) {
    ...
    printMemInfo();
#if defined(ARDUINO_ARCH_RP2040)
    shdiag::Diag::printStackInfo(Console);
    shdiag::Diag::psramPrintCapacityReport(uniMem, Console);
#elif defined(ARDUINO_ARCH_ESP32)
    // Safe ESP32 summary instead of RP-only shdiag paths
    Console.println("Unified devices:");
    for (size_t i = 0; i < uniMem.detectedCount(); ++i) {
      const auto* di = uniMem.detectedInfo(i);
      if (!di) continue;
      Console.printf("  CS=%u  \tType=%s \tVendor=%s \tCap=%llu bytes\n",
...
void setup() {
  ...
  // Important: configure mapping for the current architecture
  configureSharedSPIForMCP();
