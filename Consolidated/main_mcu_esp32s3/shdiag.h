...
// ESP32: use FreeRTOS stack watermark
...
class Diag {
  // Small cooperative yield that keeps ESP32 task watchdog happy during diag prints
  static inline void diagYield() {
#if defined(ARDUINO_ARCH_ESP32)
    delay(0);
#else
    yield();
...
public:
  // Returns an approximate free stack for the current core/task.
  // RP2040: measured from __StackBottom to current SP (if in range), fallback to heap gap.
  // ESP32: use FreeRTOS high-water mark (minimum remained unused) -> "free" bytes now is at least this.
  ...
  static inline uint32_t freeStackCurrentCoreApprox() {
    ...
    // FreeRTOS reports words. Convert to bytes.
    UBaseType_t words = uxTaskGetStackHighWaterMark(nullptr);
    // If the port returns 0 on overflow/unknown, just return 0
    if (words == 0) return 0;
    return (uint32_t)words * (uint32_t)sizeof(StackType_t);
#else
    volatile uint8_t marker;
    void* heapEnd = sbrk(0);
    if (heapEnd == (void*)-1) return 0;
  ...
  // On ESP32 prints both FreeRTOS high-water-mark bytes and a simple heap summary.
  static inline void printStackInfo(Print& out) {
    ...
    out.println(" bytes");
#elif defined(ARDUINO_ARCH_ESP32)
    uint32_t freeB = freeStackCurrentCoreApprox();
    out.print("Free Stack (task high-water): ");
    out.print(freeB);
    out.println(" bytes");

    // Add a compact heap summary (internal + PSRAM), purely informative
    ...
    out.print("Heap(total/free): ");
    out.print((unsigned)heap_total);
    out.print("/");
    out.print((unsigned)heap_free);
    out.println(" bytes");
    if (psram_total) {
      out.print("PSRAM(total/free): ");
      out.print((unsigned)psram_total);
      out.print("/");
      out.print((unsigned)psram_free);
      out.println(" bytes");
    }
#else
    out.print("Free Stack (core0 approx): ");
  ...
  // PSRAM capacity report via UnifiedSpiMem::Manager (prints only; no bus activity)
  static inline void psramPrintCapacityReport(UnifiedSpiMem::Manager& mgr, Print& out = Serial) {
    ...
    for (size_t i = 0; i < mgr.detectedCount(); ++i) {
      ...
      out.println();
      // keep prints cooperative on ESP32 without touching caller sites
      diagYield();
    }
    out.print("Banks: ");
  ...
  static inline void psramSafeSmokeTest(PSRAMUnifiedSimpleFS& fs, Print& out = Serial) {
    ...
    for (uint32_t i = 0; i < TEST_SIZE; ++i) verify[i] = (uint8_t)((testAddr + i) ^ 0x5A);
    if (!dev->write(testAddr, verify, TEST_SIZE)) {
